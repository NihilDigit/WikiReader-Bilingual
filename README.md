...
Left: target-language Wikipedia titles are used when available. Right: paragraph translations appear under the original text; if blur is enabled, tap the translation to reveal it.
...
Left: tap a word, then tap the chip for a contextual explanation. Right: double-tap a sentence for quick sentence translation.
...
Settings configure API key, target language, blur, automatic translation, and concurrency.
